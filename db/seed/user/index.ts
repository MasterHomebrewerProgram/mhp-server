  ...
  await Promise.all(
    users.map(async (user, idx) => {
      ...
      if (idx < allAwards.length) {
        ...
        console.log("----Generating " + allAwards[idx].name + " Award scoresheets")
        ...
        if (currentRank.minMixedSours) {
          const mixedSourCats = shuffle(categoryList.filter(cat => cat.cat === '28'))
        ...
        const currentCats = Object.keys(catStrings.reduce((acc, cat) => {
          acc[cat.cat] = true
          return acc
        },{}))
        ...
        if (currentCats.length < currentRank.minCats) {
          ...
          const allcats = Object.keys(categoryList.reduce((acc, cat) => {
            acc[cat.cat] = true
            return acc
          }, {}))
          const remainingCats: string[] = shuffle(allcats.filter(cat => !currentCats.includes(cat)))

          catStrings.push(...remainingCats.slice(0, numNeeded).map(targetCat => categoryList.find(cat => cat.cat === targetCat)))
        }
