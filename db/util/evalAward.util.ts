...
export default (
  ...
  scoresheets.forEach((scoresheet) => {
    ...
    if (
      scoresheet.score >= award.minScore &&
      award.categoryList.includes(categoryString)
    ) {
      if (scoresheet.approved) {
        categoriesApproved[scoresheet.Style.cat + scoresheet.Style.subcat] =
          true;
      }

      categoriesUnapproved[scoresheet.Style.cat + scoresheet.Style.subcat] =
        true;
    }
  });

  const sheetsApproved =
    Object.keys(categoriesApproved).length >= award.minRequired;

  const achieved =
    Object.keys(categoriesUnapproved).length >= award.minRequired;

  const requirements = !achieved
    ? [
        {
          description: `${award.minRequired} styles ${award.minScore}+ in list`,
          categories: award.categoryList.filter(
            (categoryString) =>
              !Object.keys(categoriesUnapproved).includes(categoryString)
          ),
          completed: Math.min(
            Object.keys(categoriesUnapproved).length,
            award.minRequired
          ),
