  ...
  const requirements = achieved ? [
    {
      ...
      categories: categoryList
        ...
        .filter(categoryString => !Object.keys(total_unapproved).includes(categoryString)),
      completed: Math.min(Object.keys(total_unapproved).length, rank.minSubcats),
    ...
    },
    {
      ...
      categories: categoryList
        .map(category => category.cat+category.subcat)
        .filter(categoryString => !Object.keys(categories_unapproved).includes(categoryString)),
      completed: Math.min(Object.keys(categories_unapproved).length, rank.minCats),
      total: rank.minCats
    },
    ... !!rank.minSlmc ? [{
    ...
    ... !!rank.minMixedSours ? [{
      ...
      categories: categoryList
        .filter(category => mixedFermentationSours.includes(category.cat+category.subcat))
        .map(category => category.cat+category.subcat)
        .filter(categoryString => !Object.keys(total_unapproved).includes(categoryString)),
